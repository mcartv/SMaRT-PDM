    ...
    const renewalResult = await client.query(
        ...
        WITH eligible AS (
            ...
            WHERE s.is_active_scholar = true
              ...
              -- Do not generate a renewal for the same semester in which
              -- the scholar's approved scholarship opening belongs.
              AND (
                    a.opening_id IS NULL
                    OR NOT EXISTS (
                        SELECT 1
                        FROM program_openings source_opening
                        WHERE source_opening.opening_id = a.opening_id
                          AND source_opening.period_id = $2
                    )
              )
        )
