...
const QUEUE_CONFIG = Object.freeze({
    pd: {
        ...
        stage: 'pending_pd',
        nextRole: 'guidance',
        nextPath: '/admin/endorsements/guidance',
    ...
    guidance: {
        ...
        nextRole: 'sdo',
        nextPath: '/admin/endorsements/sdo',
        nextTitle: 'SDO clearance pending',
    },
    sdo: {
...
function ensureRoleAccess(queueKey, actor) {
...
async function loadQueueRows(stage) {
    const { rows } = await pool.query(
        ...
        order by a.submission_date desc nulls last, es.created_at desc
        `,
        [stage]
    );
    ...
    return rows.map((row) => ({
        ...
        overall_status_label: STAGE_LABELS[row.overall_status] || row.overall_status,
        ...
        pd_decision: row.pd_status,
        guidance_decision: row.guidance_status,
        sdo_decision: row.sdo_status,
...
async function fetchQueue(queueKey, actor) {
    const config = ensureRoleAccess(queueKey, actor);
    return loadQueueRows(config.stage);
}
...
async function fetchSlipDetail(slipId, actor = null) {
    if (actor?.role) {
        ...
        if (!['admin', 'pd', 'guidance', 'sdo'].includes(role)) {
            throw createHttpError(403, 'Access denied for this endorsement slip.');
        }
    }
...
async function buildCompletedSlipPdf(detail) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        detail.stages
            .filter((stage) => ['pd', 'guidance', 'sdo'].includes(stage.key))
            .forEach((stage) => {
    ...
    if (queueKey === 'pd') {
        ...
        return {
            ...
            values: [
                ...
                actorUserId,
                remarks || null,
                action === 'approve' ? 'pending_guidance' : 'rejected',
                action === 'approve' ? 'pending_guidance' : 'rejected',
            ],
        };
    }
    ...
    if (queueKey === 'sdo') {
        ...
        return {
            ...
            values: [
                ...
                normalizedAction === 'cleared' ? 'completed' : normalizedAction,
                normalizedAction === 'cleared' ? 'completed' : normalizedAction,
            ],
        };
    }
...
async function applyStageAction(queueKey, slipId, payload, actor) {
    ...
    try {
        ...
        let pdfError = null;
        if (queueKey === 'sdo' && finalizedDetail.overall_status === 'completed') {
            try {
...
async function sendPendingDigestForRole(role) {
    ...
    const rows = await loadQueueRows(QUEUE_CONFIG[queueKey].stage);
    if (!rows.length) {
        return { sent: 0, role };
    }
    ...
    const queueUrl = `${FRONTEND_BASE_URL}/admin/endorsements/${queueKey}`;
