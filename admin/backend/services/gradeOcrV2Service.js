...
function validateManifest(artifacts) {
    ...
    if (artifact.artifact_kind !== 'original' || !['image/jpeg', 'image/png'].includes(artifact.mime_type)) {
        throw httpError(400, 'Grade Enhanced OCR requires an original JPEG or PNG capture');
    }
    if (!Number.isInteger(Number(artifact.byte_count)) || Number(artifact.byte_count) <= 0
        || !/^[a-f0-9]{64}$/i.test(String(artifact.sha256 || ''))) {
        throw httpError(400, 'Grade capture manifest is invalid');
    }
    return artifact;
}
...
async function lockRequest(client, requestId, deviceId) {
    ...
    if (request.document_key !== 'student_grade_forms' || request.ocr_version !== 'v2') {
        throw httpError(409, 'Capture upload is only available for Grade OCR V2');
    }
...
async function downloadOriginal(requestId) {
    ...
    if (!row) throw httpError(409, 'Grade Enhanced OCR capture is incomplete');
    const downloaded = await supabase.storage.from(row.bucket_name).download(row.object_path);
    if (downloaded.error || !downloaded.data) throw httpError(409, 'Grade Enhanced OCR capture is unavailable');
    const bytes = Buffer.from(await downloaded.data.arrayBuffer());
    const digest = crypto.createHash('sha256').update(bytes).digest('hex');
    if (bytes.length !== Number(row.byte_count) || digest !== row.sha256) throw httpError(409, 'Grade capture integrity check failed');
    return { ...row, bytes };
}
...
exports.completeUploads = async ({ requestId, deviceId }) => {
    ...
    const candidate = await iotOcrRequestService.completeRequest({
        ...
        templateId: 'grade_form_v2',
        fields: { ...fields, subjects: Array.isArray(result.subjects) ? result.subjects : [] },
        fieldConfidence: Object.fromEntries(FIELD_KEYS.map((key) => [key, null])),
        validationIssues: [],
        processing: { ocr_version: 'v2', pipeline_version: 'grade_v2', ocr_engine: 'enhanced_ocr', model: result.model, confidence_policy: 'nullable' },
        claimedBy: deviceId,
    });
    return candidate;
};
