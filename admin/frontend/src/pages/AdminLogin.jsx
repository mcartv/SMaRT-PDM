      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-[380px]">
          ...
          <div className="mb-8">
            ...
            <p className="text-stone-500 text-sm mt-1">Sign in to manage PDM scholarship systems</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            {/* --- INPUT: Remember Me Toggle --- */}
            <div
              className="flex items-center gap-2 cursor-pointer group w-max"
              onClick={() => setRememberMe(!rememberMe)}
            >
              <div className={`w-4 h-4 rounded border transition-colors flex items-center justify-center ${rememberMe ? 'bg-orange-800 border-orange-800' : 'bg-white border-stone-300 group-hover:border-stone-400'}`}>
                {rememberMe && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
              </div>
              <span className="text-xs text-stone-600 font-medium select-none">Remember for 30 days</span>
            </div>
            ...
            <button
              ...
              style={{ background: SB_BASE, boxShadow: isLoading ? 'none' : `0 8px 20px -6px ${SB_BASE}80` }}
            >
              {isLoading ? (
                <>
