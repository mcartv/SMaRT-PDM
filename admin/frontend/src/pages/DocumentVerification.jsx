...
export function buildExtractedData(activeDoc, application) {
  ...
  if (hasStructuredBirthFields) {
    const birthFieldDefinitions = [
      ...
      ['mother_maiden_name', 'MotherÃ¢â‚¬â„¢s Maiden Name'],
...
function deriveGradeReviewValues(rawText) {
  ...
  const period = periodText.match(
    /GRADE\s*FOR\s+THE\s+PERIOD\s*[:\-]?\s*(1ST|2ND|FIRST|SECOND)(?:\s+SEMESTER)?(?:\s+\d{4}\s*[-Ã¢â‚¬â€œ]\s*\d{4})?/i
  );
  if (period) {
    derived.semester = {
      '1ST': '1st Semester',
...
function ocrScoreLabel(candidate, key, displayedValue) {
  ...
  return String(displayedValue || '').trim() ? 'Detected' : 'Ã¢â‚¬â€';
}
  ...
  if (!src) {
    return (
      <div className="flex min-h-64 items-center justify-center rounded-lg border border-dashed border-stone-300 bg-stone-50 text-sm text-stone-500">
        Loading private Birth review imageÃ¢â‚¬Â¦
      </div>
    );
  }
          ...
          <div className={`rounded-xl border p-4 space-y-4 ${gradeReviewCompleted ? 'border-green-200 bg-green-50' : 'border-blue-200 bg-blue-50'}`}>
            ...
            <div className="space-y-3">
              {GRADE_REVIEW_FIELDS.map(([key, label]) => (
                <label key={key} className="grid gap-1 sm:grid-cols-[130px_1fr_auto] sm:items-center">
                  ...
                  <span className="whitespace-nowrap text-right text-xs font-semibold text-blue-700">
                    {String(ocrFieldValue(correctedFields?.[key]) || '').trim() ? 'Detected' : 'Ã¢â‚¬â€'}
                  </span>
                </label>
              ))}
              ...
              <div className="grid gap-1 border-t border-blue-200 pt-3 sm:grid-cols-[130px_1fr_auto] sm:items-center">
                ...
                />
                <span className="whitespace-nowrap text-right text-xs font-semibold text-blue-700">
                  {String(ocrFieldValue(correctedFields?.gwa) || '').trim() ? 'Detected' : 'Ã¢â‚¬â€'}
                </span>
              </div>
            </div>
            ...
            {!birthDiagnosticOnly && <div className="rounded-xl border border-rose-100 bg-white p-4">
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <span className="inline-flex shrink-0 whitespace-nowrap rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700">
                  {Object.values(correctedFields?.child_name || {}).some((value) => String(value || '').trim()) ? 'Detected' : 'â€”'}
                </span>
              </div>
            ...
            {!birthDiagnosticOnly && BIRTH_PARENT_FIELDS.map(([fieldKey, heading]) => (
              <div key={fieldKey} className="rounded-xl border border-rose-100 bg-white p-4">
                <div className="mb-3 flex items-start justify-between gap-3">
                  ...
                  <span className="inline-flex shrink-0 whitespace-nowrap rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700">
                    {Object.values(correctedFields?.[fieldKey] || {}).some((value) => String(value || '').trim()) ? 'Detected' : 'â€”'}
                  </span>
                </div>
                <div className="grid min-w-0 gap-3 sm:grid-cols-3">
                  {BIRTH_NAME_PARTS.map(([part, label]) => (
