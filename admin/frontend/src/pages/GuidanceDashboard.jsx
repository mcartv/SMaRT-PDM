...
function getGuidanceToken() {
  return sessionStorage.getItem('guidanceToken');
}
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const headers = {
          Authorization: `Bearer ${getGuidanceToken()}`,
        ...
        if (!statsRes.ok) {
          throw new Error(statsData.message || 'Failed to load Guidance analytics');
        }
        ...
        setError(err.message || 'Failed to load Guidance dashboard.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-xs text-stone-400 uppercase tracking-widest">Loading Guidance analytics...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 py-2" style={{ background: C.bg }}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: C.muted }}>
            Real-time view for Guidance Office.
          </p>
        </div>
      </div>
        ...
        {cards.map((card) => (
          <Card key={card.label} className="border-stone-200 shadow-none">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-4 px-4">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: card.soft }}>
                <card.icon className="w-4 h-4" style={{ color: card.accent }} />
              </div>
            </CardHeader>
            <CardContent className="px-4 pb-4">
              <div className="text-2xl font-semibold" style={{ color: C.text }}>{card.value}</div>
      ...
      <Card className="border-stone-200 shadow-none overflow-hidden">
        <CardHeader className="bg-stone-50/50 border-b border-stone-100 py-4 px-5">
          <div>
            <h3 className="text-sm font-semibold" style={{ color: C.text }}>Priority Scholar Watchlist</h3>
            <p className="text-xs mt-0.5" style={{ color: C.muted }}>Highest-risk scholar records.</p>
          </div>
        </CardHeader>
        <CardContent className="p-5 space-y-3">
          {highlightedScholars.length === 0 ? (
            <div className="rounded-xl border border-dashed px-5 py-6 text-center" style={{ borderColor: C.border, background: '#fefce8' }}>
              <p className="text-sm" style={{ color: C.muted }}>No minor or major offense records are currently flagged.</p>
            </div>
          ) : (
            highlightedScholars.map((scholar) => {
              const tone = getDisciplinaryTone(scholar.sdu_level);
              return (
                <div key={scholar.scholar_id} className="flex flex-col gap-3 rounded-xl border bg-white p-4 md:flex-row md:items-center md:justify-between" style={{ borderColor: C.border }}>
                  <div>
                    <p className="text-sm font-semibold" style={{ color: C.text }}>{scholar.student_name}</p>
                    <p className="text-xs mt-0.5" style={{ color: C.muted }}>{scholar.student_number} • {scholar.program_name}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant="outline" className="text-[10px] font-medium px-2.5 py-1 rounded-full border-none" style={{ background: tone.bg, color: tone.color }}>{(scholar.sdu_level || 'clear').toUpperCase()}</Badge>
                    <span className="text-xs" style={{ color: C.muted }}>{scholar.sdo_comment ? 'Latest comment available' : 'No comment yet'}</span>
                  </div>
                </div>
              );
            })
      ...
      <footer className="pt-6 pb-2 border-t border-stone-100">
        <p className="text-center text-[11px] text-stone-300 uppercase tracking-widest">Guidance PDM · Guidance Office Portal</p>
      </footer>
    </div>
  );
}
