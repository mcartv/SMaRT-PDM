...
function getPdToken() {
  return sessionStorage.getItem('pdToken');
}
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const headers = {
          Authorization: `Bearer ${getPdToken()}`,
        ...
        if (!statsRes.ok) {
          throw new Error(statsData.message || 'Failed to load PD analytics');
        }
        ...
        setError(err.message || 'Failed to load PD dashboard.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-xs text-stone-400 uppercase tracking-widest">Loading PD analytics...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 py-2" style={{ background: C.bg }}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: C.muted }}>
            Real-time view for Program Director.
          </p>
        </div>
      </div>
      ...
      <footer className="pt-6 pb-2 border-t border-stone-100">
        <p className="text-center text-[11px] text-stone-300 uppercase tracking-widest">PD PDM · Program Director Office Portal</p>
      </footer>
    </div>
  );
}
