...
export default function PDLogin() {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const response = await fetch(buildApiUrl('/api/auth/pd/login'), {
        method: 'POST',
      ...
      sessionStorage.setItem('pdToken', data.token);
      sessionStorage.setItem('pdProfile', JSON.stringify(data.user));
      navigate('/pd/dashboard');
    } catch (err) {
      setError(err.message || 'Unable to sign in to the PD panel.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen bg-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-3 p-10">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-white">PDM · PD</p>
        ...
        <div className="relative z-10 p-10 text-[10px] uppercase tracking-widest text-white/30">
          Separate credentials required for Program Director accounts
        </div>
      </div>
        ...
        <div className="w-full max-w-[380px]">
          <div className="mb-8">
            <div className="mb-2 flex items-center gap-2" style={{ color: COLORS.base }}>
              <ShieldCheck size={18} />
              <span className="text-[10px] font-bold uppercase tracking-widest">Authorized PD Access</span>
            </div>
            <h1 className="text-3xl font-bold text-stone-900">PD Panel</h1>
            <p className="mt-1 text-sm text-stone-500">Use your Program Director credentials to continue.</p>
          </div>
