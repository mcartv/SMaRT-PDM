...
// ─────────────────────────────────────────────────────────────
// Mock display data
// Replace later with backend/session-driven data
// ─────────────────────────────────────────────────────────────
const SESSION_LOG = [
  {
...
const ACTIVITY_LOG = [
  { action: 'Updated scholar probation status', time: 'Apr 08, 2026 · 10:14 AM' },
  { action: 'Reviewed disciplinary records', time: 'Apr 08, 2026 · 09:32 AM' },
  { action: 'Saved SDO maintenance settings', time: 'Apr 07, 2026 · 03:22 PM' },
  { action: 'Monitored flagged scholar cases', time: 'Apr 06, 2026 · 01:40 PM' },
];
...
function StatCard({ label, value, icon: Icon, tone = 'green' }) {
  ...
  return (
    <div className="rounded-2xl border border-stone-100 bg-stone-50/50 p-4">
      <div className={`w-9 h-9 rounded-xl border flex items-center justify-center ${toneMap[tone]}`}>
        <Icon className="w-4 h-4" />
      </div>
      <p className="text-2xl font-bold text-stone-900 leading-none mt-4">{value}</p>
...
export default function SDOProfile() {
  ...
  const fallbackName = profile?.name || `${firstName} ${lastName}`.trim() || 'SDO Staff';

  const sdoData = {
    firstName: firstName || fallbackName.split(' ')[0] || 'SDO',
    lastName: lastName || fallbackName.split(' ').slice(1).join(' ') || 'Staff',
    email: profile?.email || 'sdo@pdm.edu.ph',
    phone: profile?.phone || profile?.phone_number || '+63 917 000 0000',
    position: profile?.position || 'Student Disciplinary Officer',
  ...
  const fullName = `${sdoData.firstName} ${sdoData.lastName}`.trim();

  const initials = useMemo(() => {
    const a = sdoData.firstName?.[0] || '';
    const b = sdoData.lastName?.[0] || '';
    return `${a}${b}`.toUpperCase() || 'SD';
  }, [sdoData.firstName, sdoData.lastName]);
  ...
  return (
    <div className="space-y-6 py-2">
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-stone-900">SDO Profile</h1>
        ...
        <Button
          ...
          onClick={() => (window.location.href = '/sdo/maintenance')}
        >
      ...
      <Card className="rounded-3xl border-stone-200 shadow-sm overflow-hidden bg-white">
        <CardContent className="p-0">
          <div className="bg-gradient-to-r from-stone-50 to-emerald-50/60 border-b border-stone-100 px-6 py-6">
            <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
              <div className="flex flex-col md:flex-row md:items-center gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-medium text-stone-600">{sdoData.position}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 xl:w-[430px]">
                <StatCard label="Cases Reviewed" value="128" icon={FileSearch} tone="green" />
                <StatCard label="Status Updates" value="47" icon={Scale} tone="amber" />
                <StatCard label="Flagged Cases" value="9" icon={AlertTriangle} tone="red" />
                <StatCard label="Years Active" value="2" icon={Shield} tone="stone" />
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          <SectionCard
            ...
            subtitle="Read-only profile details for this SDO account."
            icon={User}
            action={
              <Badge
                variant="outline"
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InfoRow icon={Shield} label="Role" value={sdoData.role} />
              <InfoRow icon={Mail} label="Email Address" value={sdoData.email} />
              <InfoRow icon={Phone} label="Phone Number" value={sdoData.phone} />
              <InfoRow icon={Building2} label="Department" value={sdoData.department} />
              <InfoRow icon={Scale} label="Position / Role Title" value={sdoData.position} />
            </div>
          ...
          <SectionCard
            ...
            subtitle="Latest actions performed using this SDO account."
            icon={Activity}
          >
        ...
        <div className="space-y-6">
          <SectionCard
            ...
            subtitle="Current standing of this SDO account."
            icon={Shield}
          >
            <div className="space-y-3">
              <div className="rounded-2xl border border-stone-200 bg-stone-50/60 px-4 py-3">
                ...
                <p className="text-sm font-semibold text-stone-800">{sdoData.status}</p>
              </div>

              <div className="rounded-2xl border border-stone-200 bg-stone-50/60 px-4 py-3">
                <p className="text-[11px] uppercase tracking-wider text-stone-400 mb-1">Access Level</p>
                <p className="text-sm font-semibold text-stone-800">{sdoData.role}</p>
              </div>

              <div className="rounded-2xl border border-stone-200 bg-stone-50/60 px-4 py-3">
                <p className="text-[11px] uppercase tracking-wider text-stone-400 mb-1">Assigned Office</p>
                <p className="text-sm font-semibold text-stone-800">{sdoData.department}</p>
              </div>
            </div>
          </SectionCard>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-stone-500 mt-0.5">
                    Edit SDO details, monitoring settings, and audit-related maintenance records.
                  </p>
                </div>
