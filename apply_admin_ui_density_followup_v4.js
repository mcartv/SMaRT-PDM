...
/*
 * SMaRT-PDM — Admin UI Density Follow-up v4
 *
...
const PATCH_ID = 'SMART_PDM_ADMIN_UI_DENSITY_FOLLOWUP_V4';
...
function fail(message) {
  console.error('\n[ADMIN DENSITY V4] ERROR: ' + message);
  process.exit(1);
}
...
function patchLandingTheme(source) {
  ...
  const descriptionBlock = `            <p className={MAINTENANCE_CARD_SUBTITLE_CLASS}>
...
`;
  ...
  if (out.includes(descriptionBlock)) {
    out = out.replace(descriptionBlock, '');
    ...
    throw new Error(
      'Landing Theme: description text exists but expected block was not found.'
    );
  }
...
function patchReports(source) {
  ...
  if (!out.includes('compact={portalKey === \'admin\'}')) {
    ...
    out = replaceExact(
      ...
      mapAnchor,
      replacement,
      'Reports: enable compact templates for Admin only'
    );
  } else {
...
if (dryRun) {
  console.log('\n[ADMIN DENSITY V4] Dry run passed.');
...
console.log('\n[ADMIN DENSITY V4] Installed successfully.');
