...
const PATCH_NAME = 'SMaRT-PDM Mobile Endorsement Access v1';
...
function makeBackup(root, originals) {
  const backupRoot = path.join(
    ...
    `mobile-endorsement-access-v1-${Date.now()}`
  );
...
test('Endorsement screen remains the source of endorsement state', () => {
  assert.match(endorsement, /Endorsement progress/);
...
function main() {
  ...
  console.log('[1/5] Verifying the existing Endorsement route + state screen...');
  ensureIncludes(
    router,
  ...
  ensureIncludes(
    endorsement,
    ['Endorsement progress', 'final slip = endorsement.slip', 'slip.available'],
  ...
  console.log('[5/5] Building targeted navigation regression tests...');
