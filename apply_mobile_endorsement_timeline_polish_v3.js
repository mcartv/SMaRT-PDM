...
const PATCH_NAME = 'SMaRT-PDM Mobile Endorsement Timeline Polish v3';
...
function makeBackup(root, originals) {
  const backupRoot = path.join(
    ...
    `mobile-endorsement-timeline-polish-v3-${Date.now()}`
  );
...
function main() {
  ...
  console.log('[4/4] Validating timeline-specific mobile-theme and responsive contracts...');
  ensureIncludes(
    stagedScreen,
    [
      TIMELINE_MARKER,
  ...
  // Validate only timeline-specific obsolete contracts here.
  // `border: Border.all(color: outline)` is still intentionally used by
  // other Endorsement UI cards, so checking the entire screen for it creates
  // a false failure even when the new roadmap is correct.
  ensureExcludes(
    stagedScreen,
    [
      "shortLabel: 'PD'",
