...
/*
 * Update stale PDF regression expectations without depending on indentation,
 * line wrapping, or the surrounding expect(...) shape.
 *
 * The current repo test still contains the obsolete Student Number coordinate:
 *   r(721, 2180, 300, 50)
 *
 * The calibrated renderer now uses:
 *   r(420, 2230, 445, 55)
 */
const obsoleteStudentNumberCoordinate =
  'r(721, 2180, 300, 50)';
const calibratedStudentNumberCoordinate =
  'r(420, 2230, 445, 55)';

if (exportTest.includes(obsoleteStudentNumberCoordinate)) {
  exportTest = exportTest
    .split(obsoleteStudentNumberCoordinate)
    .join(calibratedStudentNumberCoordinate);
}

const obsoleteApplicationIdExport =
  'generateFromApplicationId(applicationId)';
const currentByteExport =
  'generateBytesFromSubmissionPayload(';

if (exportTest.includes(obsoleteApplicationIdExport)) {
  exportTest = exportTest
    .split(obsoleteApplicationIdExport)
    .join(currentByteExport);
}

if (exportTest.includes(obsoleteStudentNumberCoordinate)) {
  throw new Error(
    'Preflight failed: obsolete Student Number coordinate remains in pdf_export_mapping_regression_test.dart. No project files were written.'
  );
}

if (!exportTest.includes(calibratedStudentNumberCoordinate)) {
  throw new Error(
    'Preflight failed: calibrated Student Number coordinate is missing from pdf_export_mapping_regression_test.dart. No project files were written.'
  );
}

if (exportTest.includes(obsoleteApplicationIdExport)) {
  throw new Error(
    'Preflight failed: obsolete application-id PDF export expectation remains. No project files were written.'
  );
}
...
try {
  ...
  // Do not delete backup files until the COMPLETE mobile Flutter test suite passes.
  run(
    'flutter',
    ['test'],
    frontend
...
console.log(
  '\nPASS: PDF calibration + full Flutter tests passed and backup cleanup completed.'
);
console.log(
