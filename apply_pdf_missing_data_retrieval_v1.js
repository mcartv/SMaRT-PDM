...
if (!preview.includes('_submittedFormPayload = const {};')) {
  preview = replaceOnce(
    preview,
    `          _data = null;
          _application = const {};
...
const validation = [
  ...
  [printModel.includes("selectedFinancialSupport.contains('parents')"), 'Financial Support multi-select'],
  ...
  [printModel.includes('_formatDate(submittedApplicationDate)'), 'submission date retrieval'],
];
...
console.log('\nPASS: PDF missing-data retrieval + full Flutter tests passed.');
