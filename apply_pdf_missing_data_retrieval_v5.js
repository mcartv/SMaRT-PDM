...
for (const [oldText, newText] of [
  ...
  printModel = printModel.split(oldText).join(newText);
}
...
const validation = [
  ...
  [
    printModel.split('final selectedFinancialSupport').length - 1 === 2 &&
      printModel.split("supportParents: selectedFinancialSupport.contains('parents')").length - 1 === 2 &&
      printModel.split("supportScholarship: selectedFinancialSupport.contains('scholarship')").length - 1 === 2 &&
      printModel.split("supportLoan: selectedFinancialSupport.contains('loan')").length - 1 === 2 &&
      printModel.split("supportOther: selectedFinancialSupport.contains('other')").length - 1 === 2 &&
      !printModel.includes("supportParents: financialSupport.toLowerCase() == 'parents'") &&
      !printModel.includes("supportScholarship: financialSupport.toLowerCase() == 'scholarship'") &&
      !printModel.includes("supportLoan: financialSupport.toLowerCase() == 'loan'") &&
      !printModel.includes("supportOther: financialSupport.toLowerCase() == 'other'"),
...
/* ---------- TEST FILES ---------- */
const flutterTest = String.raw`import 'dart:io';
...
void main() {
  ...
  test('print model retrieves every requested PDF field from persisted form data', () {
...
try {
  ...
  if (backendTests.length > 0) {
    run('node', ['--test', ...backendTests], backend);
  }
...
console.log('\nPASS: PDF missing-data retrieval v5 + all mobile backend/Flutter tests passed.');
