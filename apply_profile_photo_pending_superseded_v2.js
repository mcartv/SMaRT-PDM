...
const PATCH_NAME = 'SMaRT-PDM Profile Photos Pending + Superseded v2';
const MARKER = 'SMART-PDM_PROFILE_PHOTO_PENDING_SUPERSEDED_V2';
...
function makeBackup(root, originals) {
  const backupRoot = path.join(
    ...
    `profile-photo-pending-superseded-v2-${Date.now()}`
  );
...
function buildService(source) {
  if (source.includes(MARKER)) {
    throw new Error('Profile photo backend already contains the v2 marker.');
  }
...
function buildFrontend(source) {
  if (source.includes(MARKER)) {
    throw new Error('ProfilePhotoQueue.jsx already contains the v2 marker.');
  }
