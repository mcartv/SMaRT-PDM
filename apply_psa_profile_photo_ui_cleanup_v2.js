...
const PATCH_NAME = 'SMaRT-PDM PSA Terminology + Profile Photos UI Cleanup v2';
const PSA_MARKER = 'SMART-PDM_PSA_BIRTH_CERTIFICATE_TERMINOLOGY_V2';
const PROFILE_MARKER = 'SMART-PDM_PROFILE_PHOTO_UI_CLEANUP_V2';
...
function makeBackup(root, originals) {
  const backupRoot = path.join(
    ...
    `psa-profile-photo-ui-cleanup-v2-${Date.now()}`
  );
...
function buildPsaTerminology(documentVerification, applicationService, documentTypes) {
  ...
  // The same legacy wording is also used by the Birth Certificate OCR
  // detection panel. These are visible labels/messages, not machine aliases,
  // so standardize them too. Lower-case matching aliases remain untouched.
  const remainingLegacyVisibleLabels = countOccurrences(
    nextDocumentVerification,
    'Birth Certificate / PSA'
  );
  if (remainingLegacyVisibleLabels > 0) {
    nextDocumentVerification = nextDocumentVerification
      .split('Birth Certificate / PSA')
      .join('PSA / Birth Certificate');
  }

  const compactVisibleLabels = countOccurrences(
    nextDocumentVerification,
    'PSA/Birth Certificate'
  );
  if (compactVisibleLabels > 0) {
    nextDocumentVerification = nextDocumentVerification
      .split('PSA/Birth Certificate')
      .join('PSA / Birth Certificate');
  }
...
function validateStaged(files, testSource) {
  ...
  ensureNotContains(
    files.documentVerification,
    ['Birth Certificate / PSA', 'PSA/Birth Certificate'],
    'PSA visible terminology validation'
  );
