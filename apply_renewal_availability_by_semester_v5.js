...
function fail(message) {
  console.error('\n[RENEWAL SEMESTER V5] ERROR: ' + message);
  process.exit(1);
}
...
function run(command, commandArgs, cwd, label) {
  ...
  // IMPORTANT:
  // Do not use shell:true for process.execPath. On Windows, Node is commonly
  // installed under "C:\\Program Files\\nodejs\\node.exe"; passing that path
  // through cmd.exe without an extra quoting layer makes cmd try to execute
  // "C:\\Program" and verification fails before Node runs.
  //
  // Only .bat/.cmd commands need the Windows command shell.
  const useShell =
...
if (dryRun) {
  console.log('\n[RENEWAL SEMESTER V5] Dry run passed.');
  console.log(
...
console.log('\n[RENEWAL SEMESTER V5] Installed successfully.');
