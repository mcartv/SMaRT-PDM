...
const PATCH_NAME = 'SMaRT-PDM Student Registry Dynamic Import v1';
...
function captureBackendBaseline(backendDir, label) {
  console.log(`\n> npm test (${label})`);
  const result = run(npmCommand(), ['test'], backendDir, { capture: true });
...
function main() {
  ...
  const backupRoot = path.join(
    ...
    `student-registry-dynamic-import-v1-${new Date().toISOString().replace(/[:.]/g, '-')}`
  );
  ...
  try {
    ...
    console.log('\n> npm run build');
    result = run(npmCommand(), ['run', 'build'], frontendDir);
