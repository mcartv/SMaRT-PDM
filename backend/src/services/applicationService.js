...
async function getLatestApplicationForStudent(studentId) {
    if (!studentId) return null;
...
async function getMyStatusSummary(userId) {
    if (!userId) {
        throw createHttpError(401, 'Authentication required.');
    }
    ...
    const application = await getLatestApplicationForStudent(student.student_id);
    ...
    return {
        ...
        application: {
            ...
            document_status: application.document_status || 'Missing Docs',
            verification_status: application.verification_status || 'pending',
            opening_id: application.opening_id,
            opening_title: application.program_openings?.opening_title || '',
            program_id: application.program_id,
            program_name: application.scholarship_program?.program_name || '',
            submission_date:
                application.submission_date || application.created_at || null,
        },
    };
}

async function getApplicationPrintData(userId, applicationId) {
    if (!userId) {
        throw createHttpError(401, 'Authentication required.');
    }
    ...
    if (application.student_id !== student.student_id) {
        throw createHttpError(
            403,
            'You are not allowed to download this application form.'
        );
    }
    ...
    const fallbackPayload = await getMyFormData(userId);
    const printData =
        Object.keys(storedPayload).length > 0
            ? {
                ...
                ...storedPayload,
                opening: {
                    ...(fallbackPayload.opening || {}),
                    ...(storedPayload.opening || {}),
                    opening_id: application.opening_id || storedPayload.opening?.opening_id || '',
                    opening_title:
                        application.program_openings?.opening_title ||
                        storedPayload.opening?.opening_title ||
                        '',
                    program_name:
                        application.scholarship_program?.program_name ||
                        storedPayload.opening?.program_name ||
                        '',
                },
                account: {
                    ...(fallbackPayload.account || {}),
                    ...(storedPayload.account || {}),
                },
            }
            : fallbackPayload;
    ...
    return {
        success: true,
        template: {
            id: 'scholarship_app_form',
            name: 'Scholarship Application Form',
            asset: 'assets/files/scholarship_app_form.pdf',
        },
        application: {
            application_id: application.application_id,
            application_status:
