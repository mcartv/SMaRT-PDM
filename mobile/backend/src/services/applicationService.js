...
async function getMySubmittedFormData(userId) {
    ...
    const { data: student, error: studentError } = await supabase
        .from('students')
        .select('student_id, current_application_id')
        .eq('user_id', userId)
        .eq('is_archived', false)
        .maybeSingle();
    ...
    if (!student?.student_id) {
        return {
            ...
            editability: {
                can_edit: false,
                reason: 'Submit an application first.',
            },
        };
    }
    ...
    if (student.current_application_id) {
        const { data, error } = await supabase
            ...
            .eq('application_id', student.current_application_id)
            .eq('student_id', student.student_id)
            .eq('is_archived', false)
            .maybeSingle();
    ...
    const applicationStatus = safeText(application.application_status)
        .toLowerCase()
        .replace(/[_-]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();

    const selectionStatus = safeText(application.selection_status)
        .toLowerCase()
        .replace(/[_-]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();

    const pendingReview = applicationStatus === 'pending review';
    const selectionNotStarted =
        selectionStatus === '' ||
        selectionStatus === 'unranked' ||
        selectionStatus === 'not started';

    const reviewStarted =
        !!application.requirements_verified_at ||
    ...
    return {
        ...
        application: {
            ...
            opening_id: application.opening_id,
            opening_title: opening.opening_title || '',
            program_id: application.program_id || opening.program_id || null,
            program_name: opening.scholarship_program?.program_name || '',
            application_status: application.application_status || null,
            document_status: application.document_status || null,
            verification_status: application.verification_status || null,
...
async function submitMyApplicationForm(userId, payload = {}) {
    ...
    if (editExistingApplication) {
        payload = { ...payload };
        delete payload.edit_existing_application;
    }
    ...
    let existingEditSnapshot = null;

    if (editExistingApplication) {
        const submittedForm = await getMySubmittedFormData(userId);

        if (
            !submittedForm?.has_application ||
            submittedForm?.editability?.can_edit !== true
        ) {
            throw createHttpError(
                409,
                submittedForm?.editability?.reason ||
        ...
        if (
            ...
            safeText(submittedForm.application?.application_id) !==
                safeText(existingApplication.application_id)
        ) {
            throw createHttpError(
                409,
        ...
        existingEditSnapshot = submittedForm.application;
    }
    ...
    if (existingApplication?.application_id) {
        const {
            ...
            .update({
                ...
                application_status:
                    editExistingApplication
                        ? existingEditSnapshot?.application_status || 'Pending Review'
                        : 'Pending Review',

                document_status:
                    editExistingApplication
                        ? existingEditSnapshot?.document_status || 'Missing Docs'
                        : 'Missing Docs',

                verification_status:
                    editExistingApplication
                        ? existingEditSnapshot?.verification_status || 'pending'
                        : 'pending',
