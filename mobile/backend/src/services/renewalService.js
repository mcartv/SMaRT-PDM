...
async function getRenewalAvailability(student) {
    ...
    if (!period) {
        return {
            is_available: false,
            reason_code: 'NO_ACTIVE_ACADEMIC_PERIOD',
            reason:
                'Renewal is currently unavailable because no academic semester is active. Please wait for OSFA to configure the current semester.',
            period: null,
            application: null,
            semester_label: '',
            school_year_label: '',
    ...
    if (!application) {
        return {
            is_available: false,
            reason_code: 'NO_APPROVED_SCHOLARSHIP_APPLICATION',
            reason:
                'Renewal is unavailable because an approved scholarship application could not be found for this scholar.',
            period,
            application: null,
    ...
    const sourcePeriodId = await getOpeningPeriodId(
        application.opening_id
    );

    if (
        sourcePeriodId &&
        String(sourcePeriodId) === String(period.period_id)
    ) {
        return {
            is_available: false,
            reason_code: 'CURRENT_SCHOLARSHIP_SEMESTER_STILL_ACTIVE',
            reason:
                `Renewal is not yet available for ${semesterLabel}${schoolYearLabel ? ` · AY ${schoolYearLabel}` : ''}. Your current scholarship application belongs to this same semester. Renewal will automatically become available when OSFA activates the next academic semester.`,
            period,
            application,
    ...
    return {
        is_available: true,
        reason_code: 'AVAILABLE_FOR_ACTIVE_SEMESTER',
        reason:
            `Renewal is available for ${semesterLabel}${schoolYearLabel ? ` · AY ${schoolYearLabel}` : ''}.`,
        period,
        application,
        source_period_id: sourcePeriodId,
        semester_label: semesterLabel,
...
function serializeRenewalAvailability(availability = {}) {
    return {
        ...
        period_id: availability.period?.period_id || null,
        periodId: availability.period?.period_id || null,

        semester_label: availability.semester_label || '',
        semesterLabel: availability.semester_label || '',

        school_year_label: availability.school_year_label || '',
        schoolYearLabel: availability.school_year_label || '',
    };
}
...
async function buildUnavailableRenewalPackage(
    ...
    const program = await getProgram(application?.program_id);
    const benefactor = await getBenefactor(program?.benefactor_id);

    const semesterLabel = availability.semester_label || '';
    const schoolYearLabel = availability.school_year_label || '';
    ...
    return {
        renewal: {
            ...
            document_status: 'Not Open',
            admin_comment: availability.reason || '',
            submitted_at: null,
            reviewed_at: null,
        },
        ...
        availability:
            serializeRenewalAvailability(availability),
    };
}
...
async function buildRenewalPackage(userId) {
    ...
    const availability =
        await getRenewalAvailability(student);

    if (!availability.is_available) {
        return buildUnavailableRenewalPackage(
            student,
            availability
        );
    }
...
exports.uploadDocument = async ({
    ...
    const student = await getStudentByUserId(userId);
    await assertRenewalAvailable(student);
