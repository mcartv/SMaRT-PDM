...
async function saveRoTimeLogProof({
  ...
  // SMART-PDM RO attendance: a previously submitted image must never be
  // accepted as a new Time-In/Time-Out proof.
  const {
    data: reusedAttendanceProof,
    error: reusedAttendanceProofError,
  } = await supabase
    .from('ro_time_log_proofs')
    .select('proof_id, log_id, proof_type, created_at')
    .eq('student_id', studentId)
    .eq('photo_sha256', photoHash)
    .limit(1)
    .maybeSingle();

  if (reusedAttendanceProofError) {
    throw createHttpError(
      500,
      `Failed to verify attendance photo uniqueness: ${reusedAttendanceProofError.message}`
    );
  }

  if (reusedAttendanceProof) {
    throw createHttpError(
      409,
      'This attendance photo was already submitted before. Take a new live camera photo.'
    );
  }
...
async function reportMyRoConflict(
  ...
  if (
    ...
    throw createHttpError(
      409,
      'A concern has already been submitted for this RO assignment and is waiting for OSFA review.'
    );
  }
  ...
  const category =
    normalizeValue(
      ...
      body.concern_category
    ) || 'Other';

  if (!allowedCategories.has(category)) {
    throw createHttpError(
      400,
      'Select a valid RO concern category.'
    );
  }
  ...
  if (!reason) {
    throw createHttpError(
      400,
      'Please provide a description for the concern.'
    );
  }
  ...
  if (reason.length < 10) {
    throw createHttpError(
      400,
      'Concern description must be at least 10 characters.'
    );
  }
  ...
  if (reason.length > 1000) {
    throw createHttpError(
      400,
      'Concern description must not exceed 1,000 characters.'
    );
  }
  ...
  const {
    ...
    await supabase
      ...
      .select(
        'ro_id, assignment_status, conflict_reason, progress_status, updated_at'
      )
      .maybeSingle();
  ...
  return {
    ...
    message:
      'RO concern submitted successfully. OSFA can now review it.',

    concern: {
      category,
      description: reason,
      status:
        updatedRo.assignment_status ||
        'Conflict Reported',
      updatedAt:
        updatedRo.updated_at ||
        now,
    },
