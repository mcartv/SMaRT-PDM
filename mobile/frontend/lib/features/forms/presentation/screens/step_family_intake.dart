...
class _StepFamilyState extends State<StepFamily> {
  ...
  String? _parentResidencyDurationError() {
    ...
    if (years == null || years < 1) {
      return 'Years as resident must be at least 1.';
    }
    return null;
  }

  List<String> _residencyYearOptions() {
    final values = List<String>.generate(120, (index) => '${index + 1}');
    final saved = parentMarilaoResidencyDurationController.text.trim();
    final savedYears = int.tryParse(saved);

    if (savedYears != null && savedYears > 0 && !values.contains(saved)) {
      values.add(saved);
