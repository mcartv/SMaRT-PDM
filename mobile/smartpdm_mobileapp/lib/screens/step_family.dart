...
class _StepFamilyState extends State<StepFamily> {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        if (constraints.maxWidth < 600) {
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: fields.asMap().entries.map((entry) => Padding(
              padding: EdgeInsets.only(bottom: entry.key < fields.length - 1 ? 16.0 : 0),
              child: entry.value,
            )).toList(),
          );
        }
        return Row(
          ...
          children: fields
              ...
              .map((entry) => Expanded(
                    child: Padding(
                      padding: EdgeInsets.only(right: entry.key < fields.length - 1 ? 16.0 : 0),
                      child: entry.value,
                    ),
                  ))
              .toList(),
  ...
  void initState() {
    ...
    fatherOccupationController = TextEditingController(text: widget.data.fatherOccupation);
    fatherCompanyController = TextEditingController(text: widget.data.fatherCompanyNameAndAddress);
    selectedFatherEducation = widget.data.fatherEducationalAttainment.isNotEmpty ? widget.data.fatherEducationalAttainment : educationalOptions[0];
    ...
    motherOccupationController = TextEditingController(text: widget.data.motherOccupation);
    motherCompanyController = TextEditingController(text: widget.data.motherCompanyNameAndAddress);
    selectedMotherEducation = widget.data.motherEducationalAttainment.isNotEmpty ? widget.data.motherEducationalAttainment : educationalOptions[0];
    ...
    guardianOccupationController = TextEditingController(text: widget.data.guardianOccupation);
    guardianCompanyController = TextEditingController(text: widget.data.guardianCompanyNameAndAddress);
    selectedGuardianEducation = widget.data.guardianEducationalAttainment.isNotEmpty ? widget.data.guardianEducationalAttainment : educationalOptions[0];

    String initialParentNativeStatus = widget.data.parentNativeStatus;
    if (initialParentNativeStatus == 'Father only') initialParentNativeStatus = 'Yes, father only';
    if (initialParentNativeStatus == 'Mother only') initialParentNativeStatus = 'Yes, mother only';
    if (initialParentNativeStatus == 'Both parents') initialParentNativeStatus = 'Yes, both parents';

    selectedParentNative = parentNativeOptions.contains(initialParentNativeStatus) ? initialParentNativeStatus : parentNativeOptions[0];
    widget.data.parentNativeStatus = selectedParentNative;

    parentMarilaoResidencyDurationController = TextEditingController(text: widget.data.parentMarilaoResidencyDuration);
    parentPreviousTownProvinceController = TextEditingController(text: widget.data.parentPreviousTownProvince);
    ...
    _bind(fatherOccupationController, (value) => widget.data.fatherOccupation = value);
    _bind(fatherCompanyController, (value) => widget.data.fatherCompanyNameAndAddress = value);
    ...
    _bind(motherOccupationController, (value) => widget.data.motherOccupation = value);
    _bind(motherCompanyController, (value) => widget.data.motherCompanyNameAndAddress = value);
    ...
    _bind(guardianOccupationController, (value) => widget.data.guardianOccupation = value);
    _bind(guardianCompanyController, (value) => widget.data.guardianCompanyNameAndAddress = value);
    _bind(parentMarilaoResidencyDurationController, (value) => widget.data.parentMarilaoResidencyDuration = value);
    _bind(parentPreviousTownProvinceController, (value) => widget.data.parentPreviousTownProvince = value);
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _row([
            _field('Occupation', TextFormField(controller: fatherOccupationController, decoration: _dec('Occupation'))),
            _field('Company Name / Address', TextFormField(controller: fatherCompanyController, decoration: _dec('Company Name / Address'))),
          ]),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          _row([
            _field('Occupation', TextFormField(controller: motherOccupationController, decoration: _dec('Occupation'))),
            _field('Company Name / Address', TextFormField(controller: motherCompanyController, decoration: _dec('Company Name / Address'))),
          ]),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 16),
          _row([
            _field('Occupation', TextFormField(controller: guardianOccupationController, decoration: _dec('Occupation'))),
            _field('Company Name / Address', TextFormField(controller: guardianCompanyController, decoration: _dec('Company Name / Address'))),
          ]),
          const SizedBox(height: 24),
          ...
          if (selectedParentNative != 'No')
            _field(
              ...
              TextFormField(controller: parentMarilaoResidencyDurationController, decoration: _dec('e.g., 20 years')),
            ),
          if (selectedParentNative == 'No')
            _field(
              'If NO, what town or provinces did they come from?',
              TextFormField(controller: parentPreviousTownProvinceController, decoration: _dec('e.g., Pampanga')),
            ),
        ],
      ),
    );
