...
class CaptureSessionTest(unittest.TestCase):
    ...
    def test_real_camera_capture_can_skip_preview_restart(self):
        ...
        camera._capture_with_picamera2 = MagicMock(return_value=(True, ""))
        camera._validate_capture = MagicMock(return_value=(True, ""))

        self.assertTrue(camera.capture_image(restart_preview=False))

        camera.stop_preview.assert_called_once()
        camera._capture_with_picamera2.assert_called_once_with(camera.capture_file)
        camera._validate_capture.assert_called_once_with(camera.capture_file)
        camera.start_preview.assert_not_called()

    def test_camera_rejects_unfocused_capture_without_success(self):
        camera = CameraController()
        camera.max_capture_attempts = 2
        camera.stop_preview = MagicMock()
        camera._capture_with_picamera2 = MagicMock(
            return_value=(False, "AUTOFOCUS_NOT_LOCKED")
        )

        self.assertFalse(camera.capture_image(restart_preview=False))
        self.assertEqual(camera.last_error_code, "AUTOFOCUS_FAILED")
        self.assertEqual(camera._capture_with_picamera2.call_count, 2)

    def test_capture_session_propagates_focus_failure(self):
        camera = FakeCamera(capture=False)
        camera.last_error_code = "AUTOFOCUS_FAILED"
        buttons = FakeButtons("left")

        result = self.run_session(camera, buttons)

        self.assertEqual(result.status, FAILED)
        self.assertEqual(result.error_code, "AUTOFOCUS_FAILED")
